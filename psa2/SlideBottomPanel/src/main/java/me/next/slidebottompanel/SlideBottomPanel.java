...
public class SlideBottomPanel extends FrameLayout {
    ...
    private static final int DEFAULT_TITLE_HEIGHT_NO_DISPLAY = 160;
    public static  int DEFAULT_PANEL_HEIGHT = 220;
...
//                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                //Log.i("event", "event" + event.getY() + " " + (mMeasureHeight - mTitleHeightNoDisplay));

                if(event.getY() < (mMeasureHeight - mTitleHeightNoDisplay) )
                    return false;

                break;
        }
        return true;//handlePanelActionDown(event);
    }
        ...
        if (isPanelShowing) {
            ...
            if (currentY < (mMeasureHeight - mPanelHeight) ||
                ...
                ObjectAnimator.ofFloat(mPanel, "y", currentY, mMeasureHeight - mPanelHeight)
                        .setDuration(isGone ? 0 : mAnimationDuration).start();
 ...
 //       /Log.i("yoyoy", "yoyo" + downY + " " + mMeasureHeight + " " + mTitleHeightNoDisplay);
        ...
        //Log.i("yoyoy", "yoyo" + isPanelOnTouch + " " + isConsume);
        return isConsume;
    }

    private boolean handlePanelActionDown(MotionEvent event) {
        boolean isConsume = false;
        mPressStartTime = System.currentTimeMillis();
        firstDownX = event.getX();
        firstDownY = downY = event.getY();
    ...
    private boolean handleActionDown(MotionEvent event) {
        ...
        firstDownY = downY = event.getY();
        //102 // 977 -980
        Log.i("yoyoy", "yoyo" + downY + " " + mMeasureHeight + " " + mPanelHeight);
        if (!isPanelShowing && downY > mMeasureHeight - mPanelHeight) {
            isPanelOnTouch = true;
            isConsume = true;
        } else if (!isPanelShowing && downY <= mMeasureHeight - mPanelHeight) {
            isPanelOnTouch = false;
    ...
    public void hidePanel() {
        ...
        mHandler.postDelayed(collapseCallback, 0);
    }
    ...
    public void displayPanel() {
        ...
        ValueAnimator animator = ValueAnimator.ofFloat(ViewHelper.getY(mPanel), mMeasureHeight - mPanelHeight)
                .setDuration(isGone? 0 :mAnimationDuration);
        animator.setTarget(mPanel);
        ...
        Handler mHandler = new Handler();
        mHandler.postDelayed(expandCallback, 0);
        //expandCallback.run();
    }
    ...
    Runnable expandCallback = new Runnable() {
        @Override
        public void run() {
    ...
    Runnable collapseCallback = new Runnable() {
        @Override
        public void run() {
    ...
    public void setExpandCallback(Runnable expandCallback) {
